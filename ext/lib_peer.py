...
def peer_message (broker_type, type_val):
    broker_type_rev_map[broker_type] = type_val 
    broker_type_map[type_val] = broker_type
    def f (c):
...
#1. Broker Header
class broker_header (object):
    _MIN_LENGTH = 8
    
    def __init__ (self, **kw):
        self.version = BROKER_VERSION
        self._xid = None
    ...
    def show (self, prefix=''):
        ...
        outstr += " (" + broker_type_map.get(self.header_type, "Unknown") + ")\n"
...
class peer_hello(broker_header):
    _MIN_LENGTH = broker_header._MIN_LENGTH
    
    def __init__ (self, **kw):
        broker_header.__init__(self)

    def pack (self):
        packed = b""
        packed += broker_header.pack(self)
        return packed
    ...
    def __eq__ (self, other):
        if type(self) != type(other): return False
        if not broker_header.__eq__(self, other): return False
        return True

    def show (self, prefix=''):
        outstr = ''
        outstr += prefix + 'header: \n'
        outstr += broker_header.show(self, prefix + '  ')
        return outstr
    
@peer_message("PEER_ERROR", 1)  # FIXME:
class peer_error(broker_header):
    _MIN_LENGTH = broker_header._MIN_LENGTH
...
class peer_echo_request(broker_header):
    _MIN_LENGTH = broker_header._MIN_LENGTH
...
class peer_echo_reply(broker_header):
    _MIN_LENGTH = broker_header._MIN_LENGTH
...
class peer_features_report(broker_header):
    _MIN_LENGTH = broker_header._MIN_LENGTH
...
def _init ():
    ...
    for k,v in globals().iteritems():
        if (k.startswith("broker_") and k.endswith("_rev_map") and type(v) == dict):
            maps.append((k[:-8],v))
    for name,m in maps:
...
#print broker_type_rev_map
#print broker_type_map
